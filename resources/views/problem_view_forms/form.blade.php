<div class="form-group {{ $errors->has('problem_instance') ? 'has-error' : '' }}">
    <label for="problem_instance"
        class="col-md-2 control-label">{{ trans('problem_view_forms.problem_instance') }}</label>
    <div class="col-md-10">
        ...
        @if (isset($problemViewForm->problem_instance) && !empty($problemViewForm->problem_instance))
            <div class="input-group input-width-input">
                <span class="input-group-addon">
                    <input type="checkbox" name="custom_delete_problem_instance" class="custom-delete-file" value="1"
                        {{ old('custom_delete_problem_instance', '0') == '1' ? 'checked' : '' }}> Delete
                </span>
    ...
    <div class="col-md-10">
        <select class="form-control" id="begin_hour" name="begin_hour" required="true">
            <option value="" style="display: none;"
                {{ old('begin_hour', optional($problemViewForm)->begin_hour ?: '') == '' ? 'selected' : '' }} disabled
                selected>La journée démarre à quelle heure</option>
            @foreach ([
        '0' => trans('problem_view_forms.begin_hour_0'),
        ...
        '23' => trans('problem_view_forms.begin_hour_23'),
    ]
    as $key => $text)
                <option value="{{ $key }}"
                    {{ old('begin_hour', optional($problemViewForm)->begin_hour) == $key ? 'selected' : '' }}>
                    {{ $text }}
                </option>
            @endforeach
        </select>
...
<div class="form-group {{ $errors->has('days') ? 'has-error' : '' }}">
    ...
    <div class="col-md-10">
        <input class="form-control" name="days" type="text" id="days"
            value="{{ old('days', optional($problemViewForm)->days) }}" minlength="1" required="true"
            placeholder="Par ordre croissant">
...
<div class="form-group {{ $errors->has('minutes_per_slot') ? 'has-error' : '' }}">
    <label for="minutes_per_slot"
        class="col-md-2 control-label">{{ trans('problem_view_forms.minutes_per_slot') }}</label>
    <div class="col-md-10">
        <input class="form-control" name="minutes_per_slot" type="number" id="minutes_per_slot"
            value="{{ old('minutes_per_slot', optional($problemViewForm)->minutes_per_slot) }}" minlength="1"
            required="true">
